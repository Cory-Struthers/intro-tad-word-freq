...
<center>![](/Users/cs86487/Dropbox/text-as-data-JUST-CORY-AND-AMBER/modules/images/bag of words.png){width="60%"}</center>
...
Some dfms are very large, involving hundreds of thousands of documents and millions of features. Features within those documents may be used rarely across the whole corpus, or used many times but only in a single document. Many text analysis methods do not infer meaning from terms mentioned very few times, and retaining all terms in really large corpora increases computational complexity (i.e., the time it takes to complete the task). Thus, the researcher has the option of "trimming" the dfm to exlude infrequent terms using `dfm_trim`. When used appropriately, trimming can decrease processing time without losing quality. In fact, reducing _sparseness_ or _sparsity_ in the matrix (i.e., the proportion of cells that have zero counts) is particularly important for more complex computational tasks in order to avoid overfitting and using memory.
...
`min_docfreq` removes any features that occur in less than _n_ times across _documents_. `docfreq_type` specifies a count or proportion.
...
Let's plot top 15 more frequent words used in Trump's tweets from Android and from iPhone.
...
**QUESTION X. How might we improve the pre-processing steps to create more meaningful output given our central research question?**
...
### Understanding and appling TF-IDF
...
**QUESTION X: Oops, what went wrong here?**
...
The grouped TF-IDF approach produces 878/970 zeros (90% of all terms), whereas a zero value isn't present (or possible) for the word frequency approach. Why does this happen? Let's calculate the TF-IDF for a common term that appears in the `tweet_words_freq`, "crooked" in both Android and iPhone.

TF = (# of times a term appears in a document) / (# of terms in a document) X
IDF = Log[(# of documents) / (# of documents containing the word)]

CORY: WILL GET THE ACTUAL VALUES AS OPPOSED TO EXAMPLE AND MAKE FORMATTING BETTER
